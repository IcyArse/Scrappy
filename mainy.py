...
if login_response.ok:
    ...
    # Now that you are logged in, scrape the redirected page content
    redirected_page_response = session.get(login_response.url)
    ...
    if redirected_page_response.ok:
        ...
        #mainelement = redirected_soup.find('div', class_='main_section')
        #sub_element = mainelement.find('form', name='t_home_form')
        #subsub_element = sub_element.find()
        
        # Find the <td> element with the given class ID
        td_element = redirected_soup.find('td', class_='class_id', string='43513647')
        print(td_element)
        if td_element:
        ...
        print('Failed to scrape the redirected page')
else:
    print('Login failed')
